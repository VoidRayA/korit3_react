...
// .env에 쓰지 않았기 때문에 VITE_로 시작하지 않았음.
...
// HATEOAS 객체를 프론트 상에서 쓸 수 있도록 미리 변환시키는 함수를 작성
...
export const addTodoApi = async(text: string): Promise<Todo> => {
  try {
    const reseponse = await axios.post<HateoasTodo>(`${API_BASE_URL}/todos`, {
      text,
      completed: false,
    });
    return formatTodo(reseponse.data);
  } catch (error) {
...
export const toggleTodoApi = async (id: number, completed: boolean): Promise<Todo> => {
  try {
    const reseponse = await axios.patch<HateoasTodo>(`${API_BASE_URL}/todos/${id}`, {completed: !completed});
    return formatTodo(reseponse.data);
  } catch (error) {
    console.log(`Error toggle todo ${id} : `, error);
    throw error;
  }
};
